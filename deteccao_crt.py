...
net = cv2.dnn.readNet("lpscr-net.weights", "lpscr-net.cfg")
classes = []
with open("lpscr-names.txt", "r") as f:
    classes = [line.strip() for line in f.readlines()]

layer_names = net.getUnconnectedOutLayersNames()

# Leitura das configurações do YOLO a partir do arquivo lpscr.data
with open("lpscr.data", "r") as data_file:
    data_content = data_file.readlines()
    for line in data_content:
        if line.startswith("names"):
            names_file = line.split("=")[1].strip()

# Caminho completo para o arquivo de nomes
names_file_path = os.path.join(os.path.dirname("lpscr.data"), names_file)
...
for img_name in images:
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            result_path = os.path.join(output_folder, f"resultado_{img_name}")
